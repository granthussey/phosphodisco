...
input_data_folder = 'corr_phospho'
input_files = [f'{phospho_prefix}_{1-na_frac_threshold:.2f}frac-no-na.top{std_quantile_threshold*100:.0f}frac_stdev.corr']
...
rule run_clusterer:
    input:
#         infile = handle_ext # '%s/%s%s' % (input_data_folder, base, file_ext)
        infile = f"corr_phospho/{phospho_prefix}_{1-na_frac_threshold:.2f}frac-no-na.top{std_quantile_threshold*100:.0f}frac_stdev.corr.csv"
    output:
...
#         readkwargs = lambda wildcards: config['read_csv_kwargs'].get(wildcards.input_file, {}),
    ...
    run:
        df = pd.read_csv(input.infile, index_col=[0,1])
...
#         input_data = handle_ext,
        input_data = f"corr_phospho/{phospho_prefix}_{1-na_frac_threshold:.2f}frac-no-na.top{std_quantile_threshold*100:.0f}frac_stdev.corr.csv",
#         readkwargs = lambda wildcards: config['read_csv_kwargs'].get(wildcards.input_file, {}),
        readkwargs = {'index_col':[0,1]},
...
#         test_labels = pd.read_csv(input[0], **params.readkwargs)
        test_labels = pd.read_csv(input[0], **readkwargs)
...
#         readkwargs = lambda wildcards: config['read_csv_kwargs'].get(wildcards.input_file, {})
    run:
        df = pd.read_csv(input.files, sep= ',', index_col=0)
